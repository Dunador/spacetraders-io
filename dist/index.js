...
class SpaceTraders {
    ...
    listLocations(system = 'OE', type, allowsConstruction) {
    ...
    jettisonGoods(shipId, good, quantity) {
        const url = this.makeUserPath(`ships/${shipId}/jettison`);
        const payload = { good, quantity };
        return this.makeAuthRequest(url, 'put', payload);
    }
    getAvailableStructures() {
        const url = `/game/structures`;
        return this.makeAuthRequest(url, 'get');
    }
    createStructure(type, location) {
    ...
    transferFromStructure(structureId, shipId, good, quantity) {
        const url = this.makeUserPath(`structures/${structureId}/transfer`);
        const payload = { shipId, good, quantity };
        return this.makeAuthRequest(url, 'post', payload);
    }
    viewStructureDetails(structureId) {
