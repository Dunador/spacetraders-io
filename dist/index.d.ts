import { AccountResponse, AvailableLoanResponse, AvailableShipResponse, FlightPlanResponse, FlightPlansResponse, LoanType, LocationResponse, LocationsResponse, MarketplaceResponse, PurchaseResponse, SellResponse, ShipResponse, ShipsResponse, ShipSellResponse, StatusResponse, JettisonResponse, AvailableStructureResponse, CreateStructureResponse, Good, StructureDepositResponse, StructureTransferResponse, ListStructuresResponse } from './types';
interface Options {
...
export declare class SpaceTraders {
    ...
    listLocations(system?: string, type?: string, allowsConstruction?: boolean): Promise<LocationsResponse>;
    ...
    jettisonGoods(shipId: string, good: string, quantity: number): Promise<JettisonResponse>;
    getAvailableStructures(): Promise<AvailableStructureResponse>;
    createStructure(type: string, location: string): Promise<CreateStructureResponse>;
    depositToStructure(structureId: string, shipId: string, good: Good, quantity: number): Promise<StructureDepositResponse>;
    transferFromStructure(structureId: string, shipId: string, good: Good, quantity: number): Promise<StructureTransferResponse>;
    viewStructureDetails(structureId: string): Promise<CreateStructureResponse>;
    listStructures(): Promise<ListStructuresResponse>;
    private createUser;
